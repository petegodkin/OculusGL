...
namespace GLSL {
	...
	int printError(std::string filename)
	{
		...
		while (glErr != GL_NO_ERROR) {
			//printf("glError in file %s @ line %d: %s\n", filename, 12345, "unknown - due to Cmake chaos");
			printf("glu - glError in file %s @ line %d: %s\n", __FILE__, __LINE__, gluErrorString(glErr));
			printf("glew - glError in file %s @ line %d: %s\n", filename.c_str(), 12345, glewGetErrorString(glErr));
			retCode = 1;
			glErr = glGetError();
		}
		return retCode;
