...
namespace GUtils {
	static void log(std::string toLog, const std::string file, int line) {
	...
	static float clampToRange(float val, float eps, float a, float b) {
		if (a < b) {
			if (val - eps <= a)
				return a + eps;
			else if (val + eps >= b)
	...
	static float clampToRange(float val, float a, float b) {
